...
from ..api.schemas.user import UserSchema,UserInSchema,PasswordSchema,UserOutSchema
...
def update(id:int,current_password:str=Body(...),new_password:str=Body(...),db:SessionLocal=Depends(get_db)):
...
def update(id:int,PasswordSchema:PasswordSchema ,db:SessionLocal=Depends(get_db)):
...
def store(UserSchema:UserInSchema,db:SessionLocal=Depends(get_db)):
    budget_repository=UserRepository.create(UserSchema,db)
    return budget_repository

@router.put("/{id}",response_model=UserOutSchema)
async def update(id:int,UserSchema:UserSchema,db:SessionLocal=Depends(get_db)):
